...
import { getActiveDraftId, generateDraftId, setActiveDraftId, clearAuthContinuation } from '@/lib/draft/storage';
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    // Priority 1: User has an existing server page
    if (data?.myPage?.id) {
    ...
    // Priority 2: User has an active draft
    const activeDraft = getActiveDraftId();
    if (activeDraft) {
      setResolvedPageId(activeDraft);
      setMode('draft');
      return;
    }
    ...
    setResolvedPageId(newDraftId);
    setMode('draft');
  }, [data, queryLoading, isFresh, router.isReady, router]);
